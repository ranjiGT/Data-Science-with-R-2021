...
d
```
The `d` dataset contains `r nrow(d)` rows and `r ncol(d)` columns.
...
Now, we will create the composite scores from the multiple choice question to be used for further analyses and visualization later in the notebook.
...
```
Similarly, we will also create composite scorces for Compliance, BFF_15 scale, SPS10 scale and Corona Concern 
...
```{r Countries_distress_score, warning=FALSE, message=FALSE, fig.height = 5, fig.width = 7}
...
### 3.1.2 World Map of Stress Levels in Countries affected with Corona

```{r World_map, warning=FALSE, message=FALSE, echo=FALSE}
dmap<-d
...
```
The stress levels marked in red hue ( level = 4 ) are highly affected areas which are less in number and the blue hue ( level = 2 ) are less affected areas which are more common in most parts of the countries.
...
cop <- d %>% summarize_at(.vars = dplyr::vars(matches("Coping_\\d")), .funs = mean, na.rm= T) %>% 
  ...
  ylab("Level of coping") + theme(axis.text.y = element_text(face = "bold")) + ggtitle("How people usually cope with Corona Stress")
ggplotly(cop)
```
...
```{r information_sources, warning=F, message=F, echo=F}
info<- d %>% summarize_at(.vars = dplyr::vars(matches("media_\\d")), .funs = mean, na.rm= T) %>% 
  pivot_longer(cols = everything(), names_to = "Source", values_to = "Value") %>% 
  mutate(Source = dplyr::recode(Source,
  ...
  ylab("Level of agreement") + theme(text = element_text(size = 12))  + theme(axis.text.y = element_text(face = "bold")) + ggtitle("Agreement on different information resources")
ggplotly(info)
```
...
```{r bivariate_realtionship_gender, fig.width = 12, fig.height = 12, warning=FALSE,message=FALSE}
library(GGally)
...
```{r bivariate_realtionship, fig.height = 12, fig.width = 12, warning=FALSE, message=FALSE}
